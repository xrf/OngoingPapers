...
CONTAINS
  ...
  !      ALLOCATE(tempPphh(nKet, nBra))
  !      ALLOCATE(vv(nBra, nBra))
  ...
  !      write(*,*) ' '
  !      write(*,*) 'pphh=',pphhBlock
  !      write(*,*) 'vHhpp=',vHhpp
       
  !      !     Matrix multiplication vHhpp*vPphh
  !      CALL DGEMM('N', 'T', nBra, nBra, nKet, 1.d0, &
  !           vHhpp, nBra, vHhpp, nBra, 0.d0, vv, nBra)
       
  !      !     Trace
  !      DO hh=1, nBra
  !         temp = temp + vv(hh, hh)
       
  !         !if (abs(vHhpp2(hh,hh)) > 0.00000001d0) then
  !         !   write(*,*) vHhpp2(hh, hh)
  !         !endif
  !      ENDDO
       
  !      DEALLOCATE(vHhpp, tempPphh, vv)
  !   ENDDO
  !   write(*,*) 'temp_pt2=',0.25d0*temp
